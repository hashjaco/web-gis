...
import { Download, FileImage, FileJson, FileSpreadsheet, Lock, Printer, Sparkles } from "lucide-react";
...
function UpgradeUpsell({ feature }: { feature: string }) {
  return (
    <div className="flex h-full w-full flex-col items-center justify-center gap-4 bg-background p-6 text-center">
      ...
      <div className="space-y-1">
        <h3 className="text-sm font-semibold">Pro Feature</h3>
        <p className="text-xs text-muted-foreground">
          <span className="font-medium capitalize">{feature}</span> is available
          on the Pro plan. Upgrade to unlock advanced GIS capabilities.
        </p>
      </div>
      <button
        type="button"
...
export function PanelContent({ activePanel }: PanelContentProps) {
  ...
  if (!canAccess(activePanel)) {
    return <UpgradeUpsell feature={activePanel} />;
  }
