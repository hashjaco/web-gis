...
export function useDeckLayers(): Layer[] {
  ...
  const { data: featureCollection } = useQuery({
    queryKey: ["features", "deck-viz", targetLayer?.id],
    queryFn: () =>
      apiFetch<FeatureCollection>(
        `/api/features?layer=${targetLayer?.id ?? ""}`,
      ),
    enabled: vizType !== "none" && !!targetLayer,
  });
